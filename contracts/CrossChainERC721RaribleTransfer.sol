...
contract CrossChainERC721RaribleTransfer {
    ...
    event TryCrossChainTransfer(uint256 toChain, address indexed nftContract, uint256 nftToken,
        address nftOwner, string nftURI, LibPart.Part[] raribleRoyalties, bytes toChainMintSignature);

    function tryCrossChainTransfer(uint256 _toChainId, address _nftContract, uint256 _nftToken, bytes memory _toChainMintSignature) external {
